...
int getprop(const char *propname, char *propvalue) {
  int rv = 0;
  int fd[2];
  int status;
  int pid;
  FILE *fp;
  const char *argv[3] = {"getprop", propname};
  pipe(fd);
  pid = fork();
  if (pid == 0) {
    close(fd[0]);
    ...
    if (status == 0) {
      fp = fdopen(fd[0], "r");
      if (fp != NULL) {
        if (fgets(propvalue, PROP_VALUE_MAX, fp) != NULL) {
          rv = 1;
        }
        fclose(fp);
      }
...
void __dns_readstartfiles(void) {
  ...
  for(i = 1; i <= MAX_DNS_PROPERTIES; i++) {
    ...
    if (getprop(propname, propvalue) > 0) {
  ...
  if (_diet_res.nscount < 1) {
      if (parsesockaddr("8.8.8.8",&_diet_res.nsaddr_list[_diet_res.nscount]))
        if (_diet_res.nscount<DIET_MAXNS)
          ++_diet_res.nscount;
  }
#else
